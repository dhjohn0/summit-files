  ...
  router.get('/folders', function (req, Folder, user, respond, views) {
    return Folder.getRoots(user).then(function (folders) {
      if (req.params.output === 'json')
        ...
        return respond(views.files.folder.get_folders, {
          folders: folders
        });
    }).catch(function (e) {
      if (req.params.output === 'json')
  ...
  router.get('/folder/:id', function (req, Folder, user, respond, views) {
    return Folder.get(
      ...
      if (req.params.output === 'json')
        ...
        return respond(views.files.folder.get_folder_id, {
          folder: folder
        });
    }).catch(function (e) {
      if (req.params.output === 'json')
  ...
  router.post('/folder', function (req, Folder, user, config) {
    return Folder.add(req.body, user).then(function (folder) {
      if (req.params.output === 'json')
        ...
        return Summit.redirect(req.params.redirect || config.files.redirects.folder.add);
      }
    }).catch(function (e) {
      if (req.params.output === 'json')
        return Summit.json({
  ...
  router.del('/folder/:id', function (req, Folder, user) {
    return Folder.remove(
      ...
      if (req.params.output === 'json')
        ...
        return Summit.redirect(req.params.redirect || config.files.redirects.folder.remove);
      }
    }).catch(function (e) {
      if (req.params.output === 'json')
        return Summit.json({
  ...
  router.put('/folder/:id', function (req, Folder, user) {
    return Folder.edit(req.params.id, req.body, user).then(function (updatedFolder) {
      if (req.params.output === 'json')
        ...
        return Summit.redirect(req.params.redirect || config.files.redirects.folder.edit);
      }
    }).catch(function (e) {
      if (req.params.output === 'json')
        return Summit.json({
